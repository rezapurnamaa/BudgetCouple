...
export default function NetworkStatus() {
  ...
  useEffect(() => {
    if (showReconnectedMessage && pendingMutations.length === 0) {
      const timer = setTimeout(() => setShowReconnectedMessage(false), 3000);
      return () => clearTimeout(timer);
    }
  }, [showReconnectedMessage, pendingMutations.length]);

  if (isOnline && !showReconnectedMessage && pendingMutations.length === 0) {
    return null;
  }
          ...
          <AlertDescription className="text-amber-800 dark:text-amber-200">
            ...
            {pendingMutations.length > 0 && (
              <span className="block mt-1 text-xs">
                {pendingMutations.length} change{pendingMutations.length !== 1 ? 's' : ''} queued
              </span>
            )}
          </AlertDescription>
        </Alert>
      ...
      {isOnline && pendingMutations.length > 0 && (
        <Alert className="bg-blue-50 border-blue-200 dark:bg-blue-950 dark:border-blue-800" data-testid="alert-syncing">
          ...
          <AlertDescription className="text-blue-800 dark:text-blue-200">
            Syncing {pendingMutations.length} change{pendingMutations.length !== 1 ? 's' : ''}...
          </AlertDescription>
        </Alert>
      )}

      {isOnline && showReconnectedMessage && pendingMutations.length === 0 && (
